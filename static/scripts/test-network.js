...
const make_test_network = data => {
    ...
    // let nodeData = d3.range(100).map((d,i) => ({r: 50 - i * .5, v: 5*d}))
    // let nodeData = data.links.map(link => ({r: link.volume, v: link.volume*10}))
    ...
    let nodeData = data.nodes.map(node => ({
        id: node.id,
...
d3.json('test-network-data.json').then(make_test_network);